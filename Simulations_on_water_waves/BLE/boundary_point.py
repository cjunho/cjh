" This file computes some values to design the computaional domain. "
...
" * Function 'yast' computes y_ast which provides the value of the bottom boundary for SP3. "
...
def bd_x1(yy,x_shift,k1,k2,k3,k4,k5,k6,t,ep,mu):
    ...
    x1 = x_shift - (k6+k5)*(4.5)**(1/6)*ep**0.5*yy + ((k6**2+k6*k5+k5**2)*(4/3)**(1/3)*ep)*t\
         - (mu/ep)**0.5*(2/9)**(1/6)*np.log(A136/A135)/(k6-k5)    
...
def bd_x2(yy,x_shift,k1,k2,k3,k4,k5,k6,t,ep,mu):
    ...
    x2 = x_shift - (k1+k2)*(4.5)**(1/6)*ep**0.5*yy + ((k1**2+k1*k2+k2**2)*(4/3)**(1/3)*ep)*t\
         - (mu/ep)**0.5*(2/9)**(1/6)*np.log(A246/A146)/(k2-k1)
    
    return x2

def bd_x21(yy,x_shift,k1,k2,k3,k4,t,ep,mu):
    x1 = x_shift - k4*(4.5)**(1/6)*ep**0.5*yy - (mu/ep)**0.5*np.log(2)/(4.5)**(1/6)/k4    
    
    return x1

def bd_x22(yy,x_shift,k1,k2,k3,k4,t,ep,mu):
    x2 = x_shift - k1*(4.5)**(1/6)*ep**0.5*yy - (mu/ep)**0.5*np.log(2)/(4.5)**(1/6)/k1
    
    return x2

def yast(k1,k2,k3,k4,k5,k6):    
    yast = (2/9)**(1/3)*0.5*np.log(k5*(k5**2-k4**2)/(k6*(k6**2-k4**2)))/(k6**2-k5**2)
