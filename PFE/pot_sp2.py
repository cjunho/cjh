...
if nic=="SP2": # SP2 solitons periodic dimensional; extend VP? Define eta and phi
    ...
    c0z = (xx22*psix1yz-xx11*psix2yz)/(xx22-xx11)
    ...
    psi_exact_exprH0 = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu_pfe))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu_pfe*((H0+eta_exact_expr)/H0)**2*(GX+GY))-(U0y*x[0]+c0y)
    psi_exact_exprz = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu_pfe))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu_pfe*((x[2])/H0)**2*(GX+GY))-(u0z*x[0]+c0z)
    sickofit =         ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu_pfe))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu_pfe*((H0+eta_exact_expr)/H0)**2*(GX+GY))-(U0y*x[0]+c0y)
