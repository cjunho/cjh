...
# python training2pressure200.py --equation NS3d --blocks 0 --file 5N19 --epochs 10 --ks 9 --filters 3 --dt 0.01  --forcing sigma5  --ndt 1 --eps 1.0 --order 1 --kind force3d --path 20_20251229T035410 --path2 10_20251229T035920
import torch
import time
import datetime
import os
...
_,oden_data0,_,En,_,_,_,_,_,_,_,_,_,Mm,Mmx,phisets,phixsets,_,_,_=matA(NN,dt,EPSILON)
...
for jj in range(NN-1):
        ...
        for ii in range(NN-1):
            ...
            oden_eye[jj,ii,]=(pre_condn[jj,ii,]@ode1)@pre_condn[jj,ii,]
...
def closure(fdata0,alp):
    ...
    # a_ex111=En.T@(ald[:,:ndt,])
    # a_ex11=torch.transpose(En.T@torch.transpose(a_ex111,2,3),2,3)
    
    # a_pred=0.01*torch.sum(ipre_condn@(a_ex11.reshape((BATCH_SIZE,ndt,NN-1,NN-1,NN-1,1))),5)/1.5

    loss_u=torch.zeros(1)
    a_pred[:,0,-1,-1,0]=0
    phial00,Pexfx = weak_pressure(alp[:,0,:ndt,],alp[:,1,:ndt,],alp[:,2,:ndt,],dt, NN,oden_eye,pre_condn, a_pred,En,Mm,Mmx )
    # Pexfx=Pexfx.reshape(BATCH_SIZE,1,NN-1,NN-1,NN-1).expand(BATCH_SIZE,nnn,-1,-1,-1).reshape(BATCH_SIZE*nnn,1,NN-1,NN-1,NN-1)
    
    a_pred111=torch.sum(pre_condn@(a_pred.reshape((BATCH_SIZE,1,NN-1,NN-1,NN-1,1))),5)
    a_pred11=En@a_pred111
    a_pred1=torch.transpose(En@torch.transpose(a_pred11,2,3),2,3)
    #loss_u1 = torch.max(abs(a_pred1[:BATCH_SIZE,:ndt,]-ald[:,3,:ndt,]))
    
    loss = 10**8*(torch.sum((phial00-Pexfx)**2))#+torch.sum((abs(a_pred-aex))**2))
...
for epoch in tqdm(range(1, EPOCHS+1)):
        ...
        loss,a_pred = closure(fdata,alp1)
...
try:
    subprocess.run(f'python inference3.py --equation NS3d  --blocks 0 --ks 9 --filters 3 --dt 0.01 --forcing {args.forcing} --ndt 1'\
                    f' --eps {EPSILON} --kind {kind} --file {sample_test} --train {FILE} --order {ORDER} --start {ORDER}', shell=True)
