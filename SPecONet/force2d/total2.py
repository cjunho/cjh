...
if os.path.isdir(data_path) == False: 
   ...
   try:
       subprocess.run(f'python ns_solver.py --case train --Nsamples 600 --Ntimes 2 --Equation {Equation}', shell=True)
       subprocess.run(f'python ns_solver.py --case test --Nsamples 100 --Ntimes 2 --Equation {Equation}', shell=True)
...
#                     f' --epochs 50000 --nbfuncs 30 --U 9 --dt 0.01 --forcing {forcing}  --ndt 1 --eps 0.1 --kind train', shell=True)
    
#     df1 = pd.read_csv(filename+f"/call1_alp.csv")
    
#     PATH=df1['path'][0]
#     subprocess.run(f'python training2alp.py --equation {Equation} --model Net3D --loss MSE --blocks 0 --file 600N23 --ks 9 --filters 10'\
#                     f' --epochs 20000 --nbfuncs 30 --U 9 --dt 0.01 --forcing {forcing}  --ndt 1 --eps 0.1 --path {PATH} --kind train --pretrained true', shell=True)
    
#     df1 = pd.read_csv(filename+f"/call1_alp.csv")
#     PATH=df1['path'][0]
#     subprocess.run(f'python training2pressure.py --equation {Equation} --model Net3Dpressure --loss MSE --blocks 0 --file 600N23 --kind train'\
...
#     PATH2=df2['path'][0]
#     subprocess.run(f'python training2pressure2.py --equation {Equation} --model Net3Dpressure --loss MSE --blocks 0 --file 600N23 --kind train'\
...
#                             f' --epochs 50000 --nbfuncs 30 --U 9 --dt 0.01 --forcing {forcing}  --ndt 1 --eps 0.1 --path {PATH} --order {ORDER} --kind train', shell=True)
#             print("Script executed successfully.")
...
#             subprocess.run(f'python training3alp.py --equation {Equation} --model Net3D --loss MSE --blocks 0 --file 600N23 --ks 9 --filters 10'\
#                             f' --epochs 20000 --nbfuncs 30 --U 9 --dt 0.01 --forcing {forcing}  --ndt 1 --eps 0.1 --path {PATH} --path_alp {PATH_alp} --order {ORDER} --kind train --pretrained true', shell=True)
